...
Every platform on the list answers how you get into the sandbox. None of them answers how the agent inside proves who it is to a third party, and the closest thing shipping today is a domain allow list, which is a firewall. So every agent calls the outside world holding some human's borrowed credential, nobody on the far end can tell an agent from its owner, and there's no revoking one sandbox out of a thousand. I work on agent identity, so discount my emphasis, but the gap is checkable in the docs.
